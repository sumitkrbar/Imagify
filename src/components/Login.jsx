...
const Login = () => {
    ...
    const onSubmitHandler = async(e) => {
        e.preventDefault(); //prevents from reloading the page on submitting
        ...
        text-slate-500'>
            ...
            {state !== "Login" && (
                <div className="flex items-center w-full border px-6 py-2 gap-2 rounded-full mt-5 h-14">
                    <img src={assets.profile_icon} className="w-9 h-9 object-cover" alt="" />
                    <input onChange={e=> setName(e.target.value)} value={name} type="text" className="outline-none text-sm flex-1 h-full" placeholder="Full Name" required />
                </div>
            )}
            ...
            <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4 h-14">
                ...
                <input onChange={e=> setEmail(e.target.value)} value={email}  type="email" className="outline-none text-sm flex-1 h-full" placeholder="Email-Id" required />
            </div>

            <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4 h-14">
                <img src={assets.lock_icon_svg} className="w-6 h-6" alt="" />
                <input onChange={e=> setPassword(e.target.value)} value={password}  type="password" className="outline-none text-sm flex-1 h-full" placeholder="Password" required />
            </div>
